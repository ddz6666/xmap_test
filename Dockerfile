...
RUN /bin/bash
# CMD ["xmap","-4","-x","32","-p","53","-M","dnsx","-O","json","-i","eth1","--output-fields=*","--output-filter=success = 1 && (repeat=0 || repeat=1)","-w","server.txt","-o","result.txt","--metadata-file=result_0724.log","-R","314","-P","4","--probe-args=raw:recurse:file:domain.txt"]